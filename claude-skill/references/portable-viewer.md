...
interactive preview; `receipt.json` is packaging evidence and is not a runtime
dependency. The HTML contains the paired files, inline Blob-module loader,
viewer modules and license notices. Its runtime uses WebCrypto SHA-256 and WebGL
in the user's desktop browser, with no external script or server dependency.
