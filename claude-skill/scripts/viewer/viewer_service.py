...
def create_server(*, assets_dir, artifacts=None, title="Architecture model",
                  ...
                  allow_open=False, opener=None, activity_provider=None, preview=None, port=0, live_state=None):
    ...
    live_state: optional trusted source LiveState; immutable checkpoint reads only.
    The caller also owns live_state.close(); browser requests never mutate it.
    ...
    if live_state is not None:
        if artifacts or preview is not None or activity_provider is not None or allow_open:
            raise ValueError("Live source state cannot mix static artifacts, providers or native opening")
        activity_provider = live_state.activity_snapshot
    assets = Path(assets_dir).resolve(strict=True)
    if not (assets / "index.html").is_file():
        raise ValueError("Viewer assets are incomplete")
    static_assets = {"index.html"}
    build_manifest_path = assets / "build-manifest.json"
    if build_manifest_path.is_file():
        build_manifest = json.loads(build_manifest_path.read_text(encoding="utf-8"))
        rows = [*build_manifest.get("files", []), *build_manifest.get("source_assets", [])]
        for row in rows:
            path = row.get("path") if isinstance(row, dict) else None
            if not isinstance(path, str) or not path.startswith("dist/"):
                raise ValueError("Viewer build manifest contains an invalid asset path")
            relative = path.removeprefix("dist/")
            if not relative or relative.startswith("/") or ".." in Path(relative).parts:
                raise ValueError("Viewer build manifest contains an invalid asset path")
            static_assets.add(relative)
    records = {}
        ...
        def do_GET(self):
            ...
            try:
                if request_path == "/api/activity":
                    ...
                    return
                if request_path.startswith("/api/checkpoints/"):
                    match = re.fullmatch(r"/api/checkpoints/([1-9][0-9]{0,8})/(preview|validation|artifacts/model)", request_path)
                    if live_state is None or match is None:
                        self._json(404, {"error": "Unknown checkpoint endpoint"})
                        return
                    revision, route = int(match.group(1)), match.group(2)
                    try:
                        data, mime = live_state.read_checkpoint(revision,
                            "artifacts" if route == "artifacts/model" else route,
                            key="model" if route == "artifacts/model" else None)
                    except ValueError as error:
                        code = 404 if str(error) in {"Unknown checkpoint", "Unknown checkpoint artifact"} else 409
                        self._json(code, {"error": str(error)})
                        return
                ...
                if request_path == "/api/model":
                    if live_state is not None:
                        self._json(200, live_state.model_snapshot())
                        return
