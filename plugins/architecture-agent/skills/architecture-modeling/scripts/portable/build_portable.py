...
def main():
    ...
    substitute('<script type="module" src="./app.mjs"></script>', '<script type="application/json" id="portable-licenses">' + json_for_html(notices) + '</script>\n<script type="application/json" id="portable-package">' + json_for_html(bundle) + '</script>\n<script type="module">\n' + bootstrap + '\n</script>')

    args.output.mkdir(parents=True)
    staging = args.output / "modules"
    for name, data in modules.items():
        target = staging / name
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(data)
    (staging / "package.json").write_text('{"type":"module"}\n', encoding="utf-8", newline="\n")
    (args.output / "payload.json").write_text(json.dumps(payload, ensure_ascii=True, indent=2) + "\n", encoding="utf-8", newline="\n")
    (args.output / "bootstrap.runtime.mjs").write_text(bootstrap, encoding="utf-8", newline="\n")
    ...
    receipt = {
        ...
        "app_adaptations": edits, "rendering_controls_changed": False, "source_geometry_generated": False,
